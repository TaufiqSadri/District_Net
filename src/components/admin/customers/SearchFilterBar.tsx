...
export default function SearchFilterBar({ paketList }: SearchFilterBarProps) {
  ...
  return (
    <section className="rounded-[18px] border border-[#e5e7eb] bg-white p-3 shadow-[0_1px_2px_rgba(15,23,42,0.05)]">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
        ...
        <FilterSelect
          ariaLabel="Filter paket pelanggan"
          value={searchParams.get('paket_id') ?? 'semua'}
          onChange={(value) => handleSelect('paket_id', value)}
          options={[
            { value: 'semua', label: 'Semua Paket' },
            ...paketList.map((paket) => ({
              value: paket.id,
              label: `${paket.nama_paket} ${paket.kecepatan_mbps} Mbps`,
            })),
          ]}
        />
        ...
        <button
          ...
          aria-label="Ubah urutan pelanggan"
        >
