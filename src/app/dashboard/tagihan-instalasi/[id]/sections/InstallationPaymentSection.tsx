...
export default function InstallationPaymentSection({
  canSubmitPayment,
  userId,
  instalasiId,
  defaultAmount,
  pembayaran,
}: InstallationPaymentSectionProps) {
  return (
    <PanelSectionCard
      title="Kirim Bukti Pembayaran"
      subtitle="Unggah bukti transfer agar pembayaran instalasi bisa diverifikasi admin."
    >
      {canSubmitPayment ? (
        ...
        <div className="rounded-xl border border-[#e5e7eb] bg-[#f8faff] px-4 py-4 text-sm text-slate-600">
          ...
          <p>
            Tagihan instalasi ini sedang diproses atau sudah lunas. Jika ada kendala, silakan hubungi admin.
          </p>
        </div>
      )}
